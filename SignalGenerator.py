...
# =============================================================================
    ...
    def setup_note2frequency(self):
        octave = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#',
                  'G', 'G#', 'A', 'A#', 'B']
        notes = [octave[note] + str(oct) for oct in range(1, 8)
...
# =============================================================================
    ...
    def generate_melody(self, idx_melody, pitch_mismatch=1, nr_tones=1):
        ...
        for idx_note in range(self.nr_notes(self.get_melody(idx_melody))):
            f = pitch_mismatch * self.note2frequency(
                self.get_note(idx_note, self.get_melody(idx_melody)))
            note = np.cos(2*np.pi * f * time_range + 2 * np.pi
                          * np.random.uniform())
            if nr_tones == 3:
                note += 1 * np.cos(2*np.pi * 3 * f * time_range + 2 * np.pi
                                   * np.random.uniform())
                note += 1 * np.cos(2*np.pi * 5 * f * time_range + 2 * np.pi
                                   * np.random.uniform())
                note *= 1/np.sqrt(3)
            melody = np.append(melody,
                               note * self.get_amplitude(idx_note,
                                                         self.UNIFORM))
    ...
    def generate_random_melody(self, snr_db, nr_tones=1):
        ...
        pitch_mismatch = self.pitch_mismatches[
            np.random.randint(self.nr_pitch_mismatches)]
    ...
    def generate_random_melodies(self, nr_generate, snr_db, nr_tones=1):
        melodies = np.zeros((len(self.generate_random_melody(snr_db)[0]),
                             nr_generate))
        ids = np.empty(nr_generate)
        mismatches = np.empty(nr_generate)
        for idx_generate in range(nr_generate):
            melody, idx_melody, mismatch = self.generate_random_melody(
                snr_db, nr_tones)
...
def print2matlabcell(M, ids, pitch):
    long_string = '{['
    long_string += ';'.join([' '.join(['{:4}'.format(item) for item in row])
                             for row in M])
    long_string += '], ['
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        sys.exit('Usage: SignalGenerator.py number-of-melodies snr-in-db' +
                 'number-of-tones')
    sg = SignalGenerator()
    melodies, true_ids, true_mismatches = sg.generate_random_melodies(
